...
| `/products/:slug` | Собственный продукт (`nexus`, `forge`, `vault`, `pulse`, `flow`, `mesh`, `drop`, `relay`, `querylab`, `workspace`) |
...
- Продукты: массив `products` в `src/data/portfolio.ts`, скриншоты в `public/images/products/*.webp` (1440×900).
...
Экраны десяти продуктов свёрстаны как статические HTML-макеты в `tools/mockups/build.py` (у каждого своя палитра, шрифт и раскладка) и сняты в Chrome на 1440×900:

```bash
python3 tools/mockups/build.py   # генерирует tools/mockups/out/*.html
# открыть out/<name>.html в Chrome на 1440×900 и сохранить как public/images/products/<name>.webp
```

Это макеты интерфейсов, а не снимки работающих систем. Когда появится реальный продукт, замените webp на настоящий скриншот и уберите статус «В разработке» в данных.

## Лоудер

Полноэкранный фирменный лоудер: `src/components/Loader.tsx` и `Loader.module.css`, подключён в `src/App.tsx`.

- Цвета и easing: переменные в начале `.root` в `Loader.module.css`; задержки анимации букв там же.
- Длительности интро, выхода и таймаут ожидания: константы `INTRO_MS`, `EXIT_MS`, `TIMEOUT_MS` в `Loader.tsx`.
- Условие завершения: функция `readinessTasks()` в `Loader.tsx` — шрифты, hero-изображение, DOMContentLoaded, load. Ошибка любого ресурса засчитывается как выполненная задача; через 8 секунд лоудер закрывается принудительно.
- Предпросмотр: открыть любую страницу с `?loader` — появится кнопка «Показать лоудер ещё раз» и демонстрационный счётчик 0–100.
- `prefers-reduced-motion`: статичный знак и короткое затухание.
